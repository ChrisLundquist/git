...
core.compressionAlgorithm::
	The compression algorithm used for new object storage. Accepted
	values are "zlib" (default) and "zstd". Requires git to be built
	with `USE_ZSTD` and the repository to have
	`extensions.compressionFormat=zstd` set (which requires
	`core.repositoryFormatVersion=1`). When set to "zstd", existing
	zlib-compressed objects remain readable via auto-detection.
