...
+
compressionFormat:::
	Specify the compression algorithm used for object storage.  The
	acceptable values are `zlib` (default) and `zstd`.  When set to
	`zstd`, git uses zstd for compressing new objects while remaining
	able to read existing zlib-compressed objects via auto-detection.
	Older versions of git that do not understand this extension will
	refuse to operate on the repository.  Requires git to be built
	with `USE_ZSTD`.
+
compatObjectFormat:::
